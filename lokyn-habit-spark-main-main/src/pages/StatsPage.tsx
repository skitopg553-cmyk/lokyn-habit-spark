...
const StatsPage = () => {
  ...
  async function loadWeekStats() {
    ...
    const result = weekDays.map(day => {
      const count = (completions || []).filter((c: any) => c.date === day.dateStr).length;
      return Math.max(Math.round((count / totalHabits) * 100), 3);
    });
        ...
        <section className="mt-10 px-6">
          ...
          <div className="flex items-end justify-between h-32 gap-2">
            {disciplineData.map((value, i) => (
              <div key={i} className="flex flex-col items-center flex-1 gap-2 relative h-full">
                {hoveredBar === i && (
                ...
                <div
                  ...
                  style={{
                    height: barsAnimated ? `${Math.max(value, 3)}%` : "0%",
