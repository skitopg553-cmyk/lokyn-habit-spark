...
const dayLabels = [
  { letter: "L", name: "Lun", key: "lun" },
  { letter: "M", name: "Mar", key: "mar" },
  { letter: "M", name: "Mer", key: "mer" },
  { letter: "J", name: "Jeu", key: "jeu" },
  { letter: "V", name: "Ven", key: "ven" },
  { letter: "S", name: "Sam", key: "sam" },
  { letter: "D", name: "Dim", key: "dim" },
];
...
const NewHabitPage = () => {
  ...
  const xp = useMemo(() => {
    ...
    else if (selectedCategory === "connaissance" || selectedCategory === "mental") base += 15;
    if (frequency === "recurring" && selectedDays.length >= 5) base += 20;
    else if (frequency === "recurring") base += 10;
  ...
  const handleCreate = async () => {
    if (!selectedCategory || !habitName.trim()) return;
    setCreating(true);

    const jours = frequency === "recurring"
    ...
    const success = await createHabit({
      ...
      frequence: frequenceDb,
      jours,
      fois_par_semaine: frequency === "weekly" ? selectedDays.length : 1,
      heure_rappel: reminderOn ? `${String(reminderHour).padStart(2, "0")}:00` : null,
      preuve_requise: proofRequired,
      xp_estime: xp,
    });
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col max-w-md mx-auto overflow-x-hidden pb-24 bg-background text-foreground font-display">
      ...
      <div className="px-4 py-6 space-y-10">
                ...
                <div className="flex justify-between w-full px-2">
                  {dayLabels.map((day, i) => (
                    <div key={i} className="flex flex-col items-center gap-2 cursor-pointer" onClick={() => toggleDay(i)}>
                      ...
                      <span className={`text-[10px] ${selectedDays.includes(i) ? "text-primary" : "text-muted-foreground"}`}>
                        {day.name}
                      </span>
                    </div>
                  ))}
        ...
        {/* Reminder */}
        <section className="p-4 bg-card rounded-xl flex flex-col gap-4">
        ...
        {/* Photo Proof Toggle */}
        <section className="p-4 bg-card rounded-xl flex items-center justify-between border border-primary/20">
          <div className="flex items-center gap-3">
        ...
        <section className="pt-8 pb-4 space-y-6">
          <div className="p-5 bg-muted rounded-2xl border border-primary/30 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10">
              <span className="material-symbols-outlined text-6xl">{selectedCat?.icon || "star"}</span>
            </div>
            <div className="space-y-3 relative z-10">
              ...
              <div className="pt-4 mt-4 border-t border-white/5 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-2 py-0.5 ${xpLevel.bgColor} ${xpLevel.color} border ${xpLevel.borderColor} rounded text-[10px] font-bold uppercase tracking-wider`}>
                    {xpLevel.label}
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground italic">Basé sur ta durée, ta fréquence et la validation requise.</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <button
              className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-4 rounded-xl shadow-[0_4px_20px_hsl(18_100%_56%_/_0.3)] active:scale-[0.95] transition-all disabled:opacity-50"
              onClick={handleCreate}
              disabled={creating}
            >
              {creating ? "Création..." : "Créer l'habitude"}
            </button>
